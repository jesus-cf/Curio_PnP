...
# media, pressure, speed, depth, cap-color, name
  ...
  ( 139,   10,     10,   1,  "red",   "Pick and Place"),
...
DEVICE = [
 ...
 { 'vendor_id': VENDOR_ID_GRAPHTEC, 'product_id': PRODUCT_ID_SILHOUETTE_CURIO, 'name': 'Silhouette Curio',
   'width_mm':  216, 'length_mm': 152, 'regmark': True },
    ...
    if no_device is True:
      ...
      for hardware in DEVICE:
        if sys_platform.startswith('win'):
          ...
          #print("device lookup under windows not tested. Help adding code!")
...
# ------------------------------------------------------------------------------------------------------------
# WARNING: in Linux you must be a member of group 'dialout' to access serial ports:
  ...
  def Init_Serial_Port(self):
    ...
    for item in portlist:
        if ('USB Serial Port' in item.description) or ('FT230X' in item.description):
            ...
            try:
                self.ser = serial.Serial(port=item[0], baudrate=115200, parity=serial.PARITY_NONE, stopbits=serial.STOPBITS_TWO, bytesize=serial.EIGHTBITS, timeout=0.5)
                time.sleep(1.5) # The Arduino is ridicusly slow at getting the serial port ready
                self.ser.write(b'.....\r\n')
                time.sleep(0.1)
                self.ser.write(b'I') # ID command.  Response from the microcontroller should be 'P'
                instr = self.ser.read(1)
                pstring = instr.decode()
                if len(pstring) > 0:
                    if pstring[0]=='P':
                        print(item[0] + " Response ok.")
                        break
                    else:
                        print("No response from " + item[0])
                        self.ser.close()
                else:
                    self.ser.close()
            except:
...
# ------------------------------------------------------------------------------------------------------------
  def move_xy(self, x_mm, y_mm):
    self.wait_for_ready(verbose=False)
    self.send_command([
      self.move_mm_cmd(y_mm, x_mm)
      ])
    self.wait_for_ready(verbose=False)

# ------------------------------------------------------------------------------------------------------------
  def draw_xy(self, x_mm, y_mm):
    self.wait_for_ready(verbose=False)
    self.send_command([
      self.draw_mm_cmd(y_mm, x_mm)
      ])
    self.wait_for_ready(verbose=False)
...
# ------------------------------------------------------------------------------------------------------------
  def addcmd(self, points):
    self.listofcmds.append(self.move_mm_cmd(points[1], points[0]))
    for j in range(0, len(points), 2):
      self.listofcmds.append(self.draw_mm_cmd(points[j+1], points[j]))

# ------------------------------------------------------------------------------------------------------------
  def sendcmds(self):
    self.wait_for_ready(verbose=False)
    #print(self.listofcmds)
    self.send_command(self.listofcmds)     
    self.wait_for_ready(300, verbose=False)
    self.listofcmds=[]

# ------------------------------------------------------------------------------------------------------------
  def Set_PnP_Time (self, onDelay=350, offDelay=350):
    try:
      self.ser.write(bytes('T'+str(onDelay)+'\r\n', 'utf-8'))
      time.sleep(1.0) # give the vacuum control some time to digest the command
      self.ser.write(bytes('t'+str(offDelay)+'\r\n', 'utf-8'))
      time.sleep(1.0) # give the vacuum control some time to digest the command
      return True
    except:
...
# ------------------------------------------------------------------------------------------------------------
  def Pick(self, x_mm, y_mm, duration=10):
    ...
    for x in range(1, duration):
        self.send_command([self.draw_mm_cmd(y_mm, x_mm)])
    self.wait_for_ready(verbose=False)
    return True

# ------------------------------------------------------------------------------------------------------------
  def Place(self, x_mm, y_mm, duration=10):
    self.send_command([self.move_mm_cmd(y_mm, x_mm)])
    self.wait_for_ready(verbose=False)
    try:
      self.ser.write(b'0')
...
# ------------------------------------------------------------------------------------------------------------
  def tool_down(self, x_mm, y_mm, tt=5):
    self.wait_for_ready(verbose=False)
    self.send_command([self.move_mm_cmd(y_mm, x_mm)])
    for x in range(1, tt):
        self.send_command([self.draw_mm_cmd(y_mm, x_mm)])
...
# ------------------------------------------------------------------------------------------------------------
  def Set_Pressure(self, Pressure=10, Tool=1):
      self.wait_for_ready(verbose=False)
      if Pressure<1:
        Pressure=1
      if Pressure>33:
        Pressure=33
      self.send_command("FX%d,%d" % (Pressure, Tool))
      self.wait_for_ready(verbose=False)

# ------------------------------------------------------------------------------------------------------------
  def Set_Speed(self, Speed=5, Tool=1):
      self.wait_for_ready(verbose=False)
      if Speed<1:
        Speed=1
      if Speed>10:
